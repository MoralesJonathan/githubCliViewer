...
module.exports = {
    ...
    runNode: (req, res) => {
        io.sockets.in(req.session.socketRoom).emit('uploadProgress', 'Running Server File...');
        terminalProcesses.push(spawn('node', [req.body.data.main], { shell: true, cwd: `downloadedRepos/${req.body.data.repo}` }));
        const terminalNum = terminalProcesses.length - 1,
            terminal = terminalProcesses[terminalNum];
        ...
        terminal.on('close', code => {
            ...
            !code === 0 ? (
                io.sockets.in(req.session.socketRoom).emit('uploadProgress', "Error running node server. Unable to continue. Please refresh this page to try again."),
                io.sockets.in(req.session.socketRoom).emit('terminalEnd', req.session.socketRoom),
                req.session.socketRoom = null,
    ...
    sendInput: (req, res) => {
        const terminal = terminalProcesses[parseInt(req.body.data[1].data)];
        terminal.stdin.setEncoding('utf-8');
        req.params.text === 'U+E007' ? (
            io.sockets.in(req.session.socketRoom).emit('terminalMessage', req.body.data[0].data),
    ...
    killProcess: (procNum) => {
        const terminal = terminalProcesses[parseInt(procNum)];
        if (terminal) {
            terminal.kill();
